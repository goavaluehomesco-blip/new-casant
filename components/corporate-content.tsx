...
export function CorporateContent({ projects, category }: CorporateContentProps) {
    ...
    <div>
      ...
      <section className="relative py-32 overflow-hidden min-h-[480px] flex items-end pb-20">
        <div className="absolute inset-0 z-0">
          {category?.image_url ? (
            <Image src={category.image_url} alt="Corporate Event" fill className="object-cover" sizes="100vw" priority />
          ) : (
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-2xl">
            <span className="text-primary font-medium tracking-wide text-sm uppercase">Corporate Gallery</span>
            <h1 className="text-5xl md:text-6xl font-bold text-white mt-4 mb-6 text-balance">
              {category?.name || "Corporate Events"}
            </h1>
            <p className="text-lg text-white/80 leading-relaxed">
              {category?.description || "Professional event production for conferences, product launches, and corporate gatherings."}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          {displayProjects.length === 0 ? (
            <div className="text-center py-24 text-muted-foreground">
              No corporate events yet. Add some from the admin panel.
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
              {displayProjects.map((project, index) => (
                <Link key={project.id} href={`/corporate/${project.slug}`}>
                  <div
