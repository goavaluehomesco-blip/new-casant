...
export default function PartnersManager({ partners: initial }: PartnersManagerProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/50 mt-0.5">Partner logos shown on the homepage</p>
        </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-white/8 bg-[#161616] py-16 flex flex-col items-center gap-3">
          ...
          <p className="text-white/40 text-sm">No partners added yet.</p>
          <Button
            onClick={openNew}
            variant="outline"
              ...
              <div className="relative h-24 bg-[#0d0d0d] flex items-center justify-center overflow-hidden">
                {item.image_url ? (
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <Handshake className="w-7 h-7 text-white/20" />
                    <span className="text-[10px] text-white/25">No logo</span>
                  </div>
                )}
                {/* Status pill */}
                <span
                  className={`absolute top-2 right-2 text-[9px] font-medium px-1.5 py-0.5 rounded-full border ${
                    item.is_active
              ...
              <div className="px-2.5 py-2 border-t border-white/8">
                <div className="flex items-center justify-between gap-1 min-w-0">
                  ...
                  <div className="flex items-center gap-0.5 shrink-0">
                    ...
                    >
                      <span
                        className={`block w-2.5 h-2.5 rounded-full border ${item.is_active ? "bg-emerald-400 border-emerald-400" : "border-white/30"}`}
                      />
                    </button>
                    <button
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. EventPro Rentals"
